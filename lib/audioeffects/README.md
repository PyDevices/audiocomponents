...
| `Notch` | **rebuilt.** Everything except one band: a tuned band-stop with a Width knob that is a bandwidth and not a depth, a Harmonics toggle that adds a second notch an octave up and half as wide in hertz for mains hum, a Depth blend whose zero is a byte-exact wire, and a make-up Trim. Five macros, six patches, zero latency. **Portability tier: audioif** (`audiobiquad`) - three float biquads, so the tail reaches exact zero where the ported node parks on 2 to 18 LSB of DC at exactly the hum settings. **What it does not do:** a `float` coefficient set cannot put the zeros exactly on the unit circle, so the centre is a true null from 500 Hz up but only −35.6 dB at 60 Hz Q 12 and −11.0 dB at 20 Hz Q 32 - a hum *reducer* at the bottom, not a hum eliminator. Budgeted at 5 % of an S3 stereo block with one notch and 9 % with two, 1.5 % / 2.5 % on the P4. Dossier `docs/effects/Notch.md` |
