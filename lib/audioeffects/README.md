...
### Drive - four at home, three still in `rebuilt/`

Phase 4's adopted classes (`Overdrive`, `Distortion`, `Bitcrusher`,
`CabinetSim`) have come home as one file per effect beside this README.
`Saturation`, `Fuzz` and `Exciter` are **board-parked on budget**: their
rebuilds stay under `lib/audioeffects/rebuilt/`, walked by the parked smoke,
and `audioeffects.create()` still serves the `drive.py` classes they
replace. `drive.py` cannot be deleted while those three remain. Every one of
the seven is **audioif** tier - none runs on a stock CircuitPython board.
