...
one instance. **Output** puts the level back, -22 to +6 dB, as SPL's own
rack module does. **Attack Speed** scales the attack detector's four time
constants together, 0.2x to 5x. **Sustain Hold** is how long the sustain
envelope holds at the peak before it lets go, 0 to 500 ms; at 0 it is a
plain follower again, which is the pre-Phase-1 behaviour.
...
followers, a peak-hold, an RMS one-pole and one multiply per frame - among
