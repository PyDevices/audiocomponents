...
import audioeffects
from audioeffects import compressor as _compressor
from audioeffects import highpass as _highpass
from audioeffects import lowpass as _lowpass
from audioeffects import parametriceq as _parametriceq

#: Every subject here is named directly rather than through
#: `audioeffects.<Name>`. The assertions below were rewritten during Phase 2
#: onto the rebuilt surfaces (`low_boost=`, a corner that clamps rather than
#: raising, a bell centre on a macro). Those four classes have come home;
#: `rebuilt.ADOPTED` no longer lists them.
Compressor = _compressor.Compressor
HighPass = _highpass.HighPass
...
class DynamicsAndEQTest(unittest.TestCase):
    ...
    def test_a_cut_and_a_boost_cost_the_same(self):
        ...
        for tag, settings in (
            ...
            with self.subTest(direction=tag):
                built = ParametricEQ(source(), **settings)
            ...
            for name in ("LowPass", "HighPass"):
                ...
                with self.subTest(filter=name, hz=hz):
                    at_corner = tone_gain_db(
                        ...
                        FILTERS[n](s, frequency=f).output)
    ...
    def test_a_low_filter_passes_what_it_should_and_stops_what_it_should(self):
        ...
        passband = tone_gain_db(
            25.0, lambda s: LowPass(s, frequency=100.0).output)
        self.assertAlmostEqual(passband, 0.0, delta=0.25)
        stopband = tone_gain_db(
            800.0, lambda s: LowPass(s, frequency=100.0).output)
        self.assertLess(stopband, -30.0)
        for hz in (30.0, 60.0):
            with self.subTest(hz=hz):
                passband = tone_gain_db(
                    hz * 8.0,
                    lambda s, f=hz: HighPass(s, frequency=f).output)
                self.assertAlmostEqual(passband, 0.0, delta=0.25)
    ...
    def test_every_band_lands_on_its_own_centre(self):
        ...
        from audioeffects._iso import ISO_BANDS
    ...
    def test_the_compressor_actually_compresses(self):
        ...
        idle = Compressor(source(), threshold_db=6.0, ratio=12.0)
        working = Compressor(source(), threshold_db=-36.0,
