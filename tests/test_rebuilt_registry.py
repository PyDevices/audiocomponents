...
#: The two fixtures under `rebuilt/`, which are not among the 46 and go at
#: Phase 7 (audiocomponents#37). Everything else `known()` returns is a real
#: rebuilt class. These tests must not care how many there are: sixteen
#: arrive in parallel, and a test that lists them by hand is exactly the
#: shared file the registry rule exists to avoid.
