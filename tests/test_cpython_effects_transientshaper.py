...
`tools/phase2_probes/transientshaper_evidence.py` and takes minutes; this
file is the fast subset a suite can carry, and every assertion here has a
faulted counterpart in the same test.

`docs/effects/TransientShaper-evidence.md` is the record; the numbers there
...
def render(pcm, frames, cls=None, macros=None, rate=RATE, channels=2):
    source = probes.ArraySource(pcm, rate=rate, block=256,
                                channels=channels)
    effect = (cls or audioeffects.TransientShaper)(source, sample_rate=rate)
...
class SurfaceTests(unittest.TestCase):
    ...
    def test_the_surface_is_the_dossier_s(self):
        cls = audioeffects.TransientShaper
        self.assertEqual(cls.__module__, "audioeffects.rebuilt.transientshaper")
    ...
    def test_the_class_reads_no_transport(self):
        ...
        source = os.path.join(ROOT, "lib", "audioeffects", "rebuilt",
                              "transientshaper.py")
    ...
    def test_patch_0_is_the_constructor_s_defaults_on_the_grid(self):
        effect = audioeffects.TransientShaper(
