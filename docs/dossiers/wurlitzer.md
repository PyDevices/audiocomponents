...
| 2 | Attack is fast and reed-like | **measurable now**, no numeric target sourced | Default `amp_a` ≈ **10.0 ms** (patch 0 raw 47 → `logmap(47/127, 0.001, 0.501)`, `wurlitzer.py:59,123`). Corrected 2026-09-03: this read "≈ 8.9 ms (patch 0 raw 2 → `0.001+2/127*0.5`)", which described the linear macro grid this instrument no longer uses. The macros were re-mapped logarithmically (audioif af837de, replayed here), and patch 0 was re-derived against it: raw 2 became raw 47 and the value it produces moved 8.87 → 9.98 ms. The declared intent was always 10 ms; the linear grid had no step near it, so the shipped value missed by 11%. Every part of the old parenthetical — the value, the raw macro, and the formula — was stale. The DAFx paper describes the reed strike but no clean millisecond attack constant was extracted from its text this session (§1) — this row needs either a re-read of the paper's figures/equations directly (not just its extracted text) or a captured onset, neither done this pass. |
