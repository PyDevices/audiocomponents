...
| D2 | **T1's *total* rejection does not survive `float` coefficients at low f₀, and the class ships anyway.** On-centre, commensurate probe, level 6000, 48 kHz, **measured**: **−35.65 dB at 60 Hz Q 12**, **−11.21 at 20 Hz Q 32**, under 1 LSB from 500 Hz up at Q ≤ 12 — against **predictions** of −35.54 and −10.98 from the node's own coefficients. §5's law, read off the node's own `coefficients`, tracks every figure to 1.5 dB — arithmetic, not a build error. **T1 is recorded disconfirmed below ~250 Hz** | A16 |
