...
- [ ] Tier 3 cost measured on the P4 and the S3. **Not run — no board leg
      this session** (§4, §11).
...
$ .venv/bin/python tools/measure_effect_cost.py --subject GraphicEQ --port <COMn>
...
- **No board leg.** Neither the ESP32-P4 nor the ESP32-S3 has run this class.
  §4's budget is arithmetic from audioif's own instruction counts, not a
  measurement, and the per-block overhead of a **twelve-node chain** is not
  in it — the desktop cannot see that cost, because the desktop's
  `audiobiquad` is a Python shim over the same kernel the board runs
  natively. This is the largest gap in the pack.
