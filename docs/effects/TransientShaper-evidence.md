...
|---|---|---|
| T1 | **refutation stands** → disconfirmed as stated | §1's narrowing ("holds only over −6…−40 dBFS") does not hold at the setting the row is measured at: point-by-point at Attack **+12** over −6 / −20 / −40 the spread is **3.422 dB** against a 0.500 bar. What holds is −20…−40 (0.163 dB) at any Attack, and −6…−40 only where the render does not clip (Attack ≤ +6, spread 0.160 dB). The headline *peak* readout is also one setting's: at Attack **−12** the peak spread over −6…−60 is **1.395 dB**. |
| T3 (Sustain) | **refutation stands** → disconfirmed | −24.0 dB is a figure of the note, not of the control. Same setting, material varied inside the trait's own terms: 10 dB/s 3 s **−24.321**; 10 dB/s 6 s **−26.846**; 5 dB/s 6 s −24.340; 3 dB/s at 2 / 3 / 6 / 12 s **−10.787 / −12.950 / −15.442 / −16.353**. On a 3 dB/s note the control saturates **8 dB short of its stated end** — T3's own disconfirmer — and the 6 s reading over-runs the other way. The Attack leg is hardened, not broken: at `hit_levels_-20` Attack +15 reads **+15.000 dB with 0 samples clipped**. |
| T4 | **refutation stands** → unmeasured | The fault is red only at the render length the pack used. Decay rates 10 and 40 dB/s exactly as frozen, varying only the render length the trait does not state: 0.5 s class **1.25:1 (fails)**; 1.0 s class 2.43 / fault **1.92 RED**; 1.5 s, 2.0 s and 3.0 s class 2.61–2.62 / fault **2.31–2.32, still green**. At 1.0 s the class's own t90 is 902 ms in a 1000 ms render. The monotone clause is worse: a plain follower's drop is **0.000 dB** at 200, 400 and 800 ms windows, so it passes the clause outright. |

T2 survives and was strengthened (43 of 203 at a second material, 42 of 204 at
44.1 kHz, 45 of 210 at 22.05 kHz, fault 0 on every one).
