...
**Not run — no ESP32-P4 or ESP32-S3 was attached to this session.** Nothing in
this section is filled from an estimate.
...
- [ ] Tier 3 cost measured on the P4 and the S3 — **not run**, §4.
...
$ audiocomponents/.venv/bin/python tools/measure_effect_cost.py --subject TransientShaper --port <COMn>
not run - no board attached to this session (section 4)
```
...
- **Tier 3 cost is not measured.** No ESP32-P4 or ESP32-S3 was attached to
  this session, so §4's table is empty and the P4/S3 digest columns in §3 are
  empty. The class gate is not met until that run happens. What it takes: a
  board on a port and `tools/measure_effect_cost.py --subject TransientShaper
  --port <COMn>`, at patch 1 rather than at the defaults (§4).
