...
|---|---|---|
| D1 | **refutation stands** → disconfirmed | The 0.798 dB spread is Sensitivity 34's, not the class's. The same four probes at **Sensitivity 44 — the setting this pack uses for D7** — read −17.682 / −17.693 / −17.834 / −20.350 dB, spread **2.668 dB** against a 1 dB bar (44.1 kHz 2.806; 22.05 kHz 1.737). The unity control on `sibilant_-55` is −0.125 dB, so the measurement floor accounts for 0.13 dB of it and not 2.7. |
| D2 (tracking) | **refutation stands** → unmeasured | The measurement cannot fail. At Range 20 / Sensitivity 0 the gain cell passes unity, so the render is the flat LR4 sum — D8's reading, 0 dB at every frequency and every corner. Holding the tone at 2500 Hz and sweeping the corner 800 → 8000 Hz gives −0.0009 … −0.0010 dB, no movement; and a planted fault with the crossover frozen at 2500 Hz **still passes** the test's own assertion. |
| D3 | **refutation stands** → the 12 dB clause disconfirmed | The frozen trait lists "either band's reduction more than 0.5 dB from the commanded 12 dB" as a disconfirming condition and the evidence table dropped it without declaring the drop. Reachable reduction saturates: Sensitivity 34 / 44 / 54 / 64 / 80 / 100 give −9.580 / −11.162 / −11.463 / −11.463 / −11.463 / −11.463 dB, so the best any reachable setting reaches is **0.537 dB from the commanded 12**; HF-only is worse (−10.913). Same asymptote cause as D7 (N-DEESS-6). The two mode clauses stand. |
| D4 (rate) | **refutation stands** → unmeasured | Nothing guards it: the test asserts only `deviation > 0.3` (the shape); the rate is printed and never asserted. Rebuilt with the Release macro moved, the D4 test is **green** at 602 dB/sec (34.9 % off), 322 (65.2 %) and 132 dB/sec (85.7 %). The 935 dB/sec is also window-dependent — 912 / 935 / 989 dB/sec at hops 1.25 / 2.5 / 5.0 ms — because an exponential has no dB/sec. The shape half stays disconfirmed. |

D5, D6 and D8 survive; D5 and D6 each owe the rate caveat the Refutation
record states (D5's rig has no resolution at 22.05 kHz; D6's bin is incoherent
at 44.1 kHz and reads a *gain* through a path whose gain cannot exceed unity).
