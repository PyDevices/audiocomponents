...
the command that produced it, is in §12. **Tally after the gate audit: 6 demonstrated, 12 disconfirmed, 3 unmeasured** *(was 10 / 8 / 3 before the refutation pass; F4, F5, V3 and V6 moved to disconfirmed)* — counted by row,
...
| V6 | Clean at any amount of compression | **disconfirmed** — REFUTED; clean at Release slowest only | SPECTRUM, 48 kHz, cpython. **0.0076–0.0125 %** at 3, 10 and 20 dB of GR across Release slowest / centre / fastest. Bar 0.2 % | the same SPECTRUM readout goes red at 3.89 % on F5's fault, so the measurement fires |
...
|---|---|---|
| F3(c) | **refutation stands** → (c) unmeasured | The +13.4 dB is the Attack/Release difference, not the mode: patch 5 given patch 2's times reads 2.0406 %, patch 2 given patch 5's times 0.5147 % — the readings swap with the times. Same confound §1 already declared for (b). |
| F4 | **refutation stands** → disconfirmed as a trait of *this class* | The 8.573 / 9.334 / 9.568 / 9.746 dB is a bare `audiodynamics.Dynamics(feedback_detector=True)` (`tools/compressor_probes.py:201-227`). Auditor's check: `grep -n feedback_detector lib/audioeffects/rebuilt/compressor.py` returns **one** hit, prose at `:44`; `_OVERRIDES` (`compressor.py:382-388`) carries no topology key and §6 leaves `Topology` off the surface. No configuration of `Compressor` reaches the tap. A palette finding (with N-C4b), not this class's. |
| F5 | **refutation stands** → disconfirmed | "Everywhere" was measured at Release slowest only. At 10 dB GR, 50 Hz: patch 1 **0.0209 / 0.2954 / 3.8909 %**, patch 5 **0.0190 / 0.2288 / 2.0828 %** at Release slowest / centre / fastest, bar 0.5 %. Release 127 is a dialable macro position, so §12's "planted fault" for this row is the class's own behaviour at an ordinary setting. |
| O1 | **refutation stands against §1's fault; the trait survives** → demonstrated on the refuter's fault | Auditor's check: `sed -n '331,333p' lib/audioeffects/rebuilt/compressor.py` shows `memory = OPTICAL_MEMORY` forced on the optical character, macro 6 never read — so §1's "Memory 0" fault is inert (t50 74.9 ms, t95 2187.2 ms, ratio 29.22, identical to the clean run). The refuter's fault of the same kind, `OPTICAL_MEMORY = 0.0`, reads t50 65.2 / t95 136.4 / ratio **2.09 → RED**. §1's fault column must be re-recorded; the 1.24 dB deviation §12 files is the level search, not the class. |
| V3 | **refutation stands** → disconfirmed below about 200 Hz | The row fixes no probe frequency and `case_v3` (`tools/compressor_evidence.py:573-591`) hardcodes 200 Hz. **Re-run by the auditor** (`scratch/audit/v3.py`, 48 kHz, cpython): \|sq−sn\| on patch 3 reads **0.079 / 0.158 / 0.322 / 0.629 dB** at 400 / 200 / 100 / 50 Hz against a 0.50 dB bar, and the peak-detector control holds at **2.914–2.982** against the derived 3.01 at every one, so the probe is still reading the detector law. \|sn−pl\| stays in band (6.257–6.863). |
| V6 | **refutation stands** → disconfirmed | Two ways, both re-read against §1: the fault §1 cites is F5's and reads **0.0107–0.0113 %** in V6's own terms (18× inside the 0.2 % bar, i.e. green); and at 50 Hz the Release-fastest column reads **0.5603 / 0.5787 / 0.5803 %** at 3 / 10 / 20 dB of GR. Clean at Release slowest (0.0095–0.0106 %). |

**Ten of sixteen demonstrated clauses survive.** The tally line under the
table and §1's closing note ("**No refutation pass ran**") are superseded by
this block and by the Refutation record: the pass ran, and it was independent.
...
An independent pass, run against the branch as it stands. Every figure below
was produced by this session on
`audiocomponents/.venv/bin/python` with
`PYTHONPATH=audiocomponents/lib`, at 48 kHz stereo unless the line says
otherwise; the fault runs monkeypatch module constants in a scratch script
and **the class was not edited**. One line per row §1 marks *demonstrated*.

| Row | Verdict | The argument |
|---|---|---|
| F1 | **stands** | Re-run at 48 kHz reproduces (0.8180 ms, 1095.3 ms, 49.7 ms). Varied the rate, which the row's own milliseconds should be blind to: at **44100 Hz** the three published ends read **0.8190 / 1097.4 / 49.4 ms**, every one in band, and the attack column is monotone across all six settings. Fault fires as recorded. |
| F2 | **stands, and the fault §1 declined to run does fire** | §1 says "with it removed both columns are flat by construction — not run as a separate fault". Run: setting `FET_RATIO_TILT_DB = 0` alone leaves the knee column *still narrowing* (12.00 / 8.00 / 6.50 / 5.00), because `compressor.py:308` scales the knee inside the same branch on a separate line. The fault has to disable the whole branch (`FET_RATIO_REFERENCE = 1e9`), and then both columns are flat — threshold **−33.01** and knee **12.00** at all four ratios, against the clean −33.01/−31.26/−30.01/−28.76 and 12.00/8.00/7.50/5.50. Red. The row is sound; its stated reason was half right. |
| F3(a) | **stands** | The 0.0001 dB/dB margin is not an artifact of the level window: read over `> −20, −18, −16, −14, −12, −10, −8 dBFS` the mean slope is **0.0502, 0.0502, 0.0502, 0.0501, 0.0503, 0.0502, 0.0501** — in band every time. Note for the author: the per-step slopes scatter **0.0496–0.0506**, five times the margin the row is claimed at, so "inside by 0.0001" is inside the reading's own noise. |
| F3(c) | **REFUTED** | The +13.4 dB is the **time constants, not the mode**. At 12 dB of GR, 100 Hz, the pack's own probe: patch 2 as shipped **2.2167 %**, patch 5 as shipped **0.4706 %**; give patch 5 patch 2's Attack and Release (macros 3 and 4 to 127) and it reads **2.0406 %**; give patch 2 patch 5's times (Attack 36, Release 60 ms) and it reads **0.5147 %**. The two readings swap with the times. This is the same confound §1 already caught in F3(b) — patch 2 and patch 5 differ in Attack as well as in mode — and it disposes of (c) as well as (b). **The author must answer:** an All-Button reading taken against a 4:1-side patch held at the *same* Attack and Release, or (c) goes to `unmeasured` beside (b). |
| F4 | **REFUTED as a trait of this class** | The measurement is real and reproduces (8.573 / 9.334 / 9.568 / 9.746 dB against the algebra), but it is taken on a bare `audiodynamics.Dynamics(feedback_detector=True)` in `compressor_probes.py:201-227`, and **no configuration of `Compressor` can reach it**: `grep -n feedback_detector lib/audioeffects/rebuilt/compressor.py` returns one hit, prose at line 44; `_OVERRIDES` carries no topology option and §6 says `Topology` was deliberately left off the macro surface. So the row demonstrates a property of the palette node, not of the class under test — which is also why O5 and M4 are disconfirmed. **The author must answer:** re-file F4 as a palette finding (with N-C4b) or state plainly that the class does not exercise it. |
| F5 | **REFUTED** | The row says "clean **everywhere** except All-Button" and is measured at Release slowest only. Swept the Release knob on the row's own two patches, same probe, same 10 dB of GR (bar 0.5 %): patch 1 → **0.0209 / 0.2954 / 3.8909 %** at 50 Hz for Release slowest / centre / fastest; patch 5 → **0.0190 / 0.2288 / 2.0828 %**. Release 127 is a macro position a player can dial, not an injected defect, so what §12 files as F5's *planted fault* is the class's own behaviour at an ordinary setting. **The author must answer:** either narrow the row to "clean at Release slowest", or find a different planted fault and record the 50 Hz / fast-release readings as a disconfirmation. |
| O1 | **REFUTED** | §1's planted fault is "Memory 0, which collapses the build to one stage". Run: patch 0 with macro 6 at 0 gives **t50 74.9 ms, t95 2187.2 ms, ratio 29.22** — identical to the clean run, to the tenth of a millisecond. The cause is in the class: `compressor.py:331-333` forces `memory = OPTICAL_MEMORY` on the optical character and never reads macro 6, so the fault is inert on the character O1 is measured on. The control §1 cites instead ("F1's release column shows the single-stage t95/t63 at 1.51") is a different statistic on a different character. A fault that does reach the stage — `OPTICAL_MEMORY = 0.0` — reads **t50 65.2 ms, t95 136.4 ms, ratio 2.09 → RED**, so the row is recoverable, but as filed it is demonstrated with no fault shown red. Also, at that fault the level search lands on **9.96 dB** of GR, not the clean run's 11.24 — the 1.24 dB deviation §12 records is the search, not the class. **The author must answer:** re-record O1's fault as `OPTICAL_MEMORY = 0`. |
| O3(c) | **stands** | Reproduces exactly: patch 4 **−0.1412 dB/dB** against patch 0 **+0.0147**. Note for the author: a negative asymptote means output *falls* as input rises, which is steeper than the row asks in a direction no limiter has; the clause is met by a number whose sign is a defect. |
| O4 (minimum) | **stands** | Reproduces: 7.02 against 7.05 dB, 0.03 dB apart on a 1 dB bar. The measurement can fail — the maximum row of the same readout reads 2.01 dB — so it is not an instrument that only ever agrees. |
| V2 | **stands, and it does transfer to the class** | §12 measures it on a bare node. Re-measured through `Compressor` (character `vca`, the only one that sets `program_attack` at `compressor.py:355`, Detector toggled to peak, ratio 1000, threshold −40): at the probe's asked 5 ms the class reads **8.083 / 3.875 / 2.000 ms**, only one of three inside V2's bands, because `compressor.py:325` divides the asked attack by `ATTACK_MEASURED_PER_SET`. At **7.7 ms** asked, the same class reads **12.708 / 6.125 / 3.167 ms** — all three in band — with **t30/t10 0.249**, and the RMS control reads 0.709. The trait holds on the shipped class; the row's three figures are the node's. |
| V3 | **REFUTED** | Varied the probe frequency, which the detector law is blind to (the row fixes no frequency; the pack chose 200 Hz). \|sq−sn\| on patch 3, bar ≤ 0.50 dB: **400 Hz 0.079, 200 Hz 0.158, 100 Hz 0.322, 50 Hz 0.629 → RED**. The peak-detector control holds at every frequency (2.914–2.982 against 3.01), so the probe is still measuring the detector law — the RMS window simply cannot null a 50 Hz square. \|sn−pl\| stays in band throughout (6.257–6.863). **The author must answer:** state the frequency floor the 0.5 dB bar holds above, or re-cut the row's window. |
| V4 (slope) | **stands** | Held at other probe frequencies: worst local slope **0.0006 at 50 Hz** and **0.0017 at 1 kHz** against the 200 Hz run's 0.0010, bar 0.05. Fault fires as recorded. Note for the author: the *knee* half — disconfirmed at 2.00 dB — fits **0.00 dB at 50 Hz** and 2.00 dB at 1 kHz, so V4's knee disconfirmation is probe-frequency dependent and worth re-reading before it is treated as a class defect. |
| V5 | **stands, and now on the class** | §12 argues it from F4's node-pair control column. Measured through `Compressor` instead, character `vca`, knee 0, threshold −30, DC step 20 dB over: **15.003 / 17.505 / 18.336 / 19.005 dB** of GR at 4:1 / 8:1 / 12:1 / 20:1, against the feed-forward fraction 15.000 / 17.500 / 18.333 / 19.000 and a feedback ceiling of 8.571 / 9.333 / 9.565 / 9.744. The shipped class is on the feed-forward side by 6–9 dB. |
| V6 | **REFUTED** | Two ways. (i) The planted fault §1 cites — "the same SPECTRUM readout goes red at 3.89 % on F5's fault" — is green in V6's own terms: patch 3, 1 kHz, Release fastest reads **0.0113 / 0.0109 / 0.0107 %** at 3 / 10 / 20 dB of GR, 18× inside the 0.2 % bar. That reading is F5's, on a different character at a different frequency. (ii) Moving only the probe tone to 50 Hz, which F5 already uses, the row's own Release-fastest column reads **0.5603 / 0.5787 / 0.5803 %** — RED, ~3× over the dbx's 0.2 %. Release slowest at 50 Hz stays clean (0.0095–0.0106 %), so the offender is the Release knob, not the frequency alone. **The author must answer:** the same one F5 owes — the row is clean at Release slowest, not "at any amount of compression" as the knob stands. |
| M2 (attack) | **stands** | §1 argues the fault by analogy ("F1's calibration fault is the same kind"). Run on M2's own six patches: `ATTACK_MEASURED_PER_SET = 1.0` takes the column from **0.232 / 0.232 / 0.479 / 0.479 / 0.479 / 0.232 ms** to **0.349 / 0.349 / 0.727 / 0.727 / 0.727 / 0.349 ms**, six of six outside the manual's ±30 % → RED. |
| M5 | **stands, and now has a fault of its own** | Varied the probe tone to 50 Hz, unchanged otherwise: TC1–TC6 read **0.2789 / 0.1059 / 0.0492 / 0.0320 / 0.0511 / 0.2587 %**, all inside the 1 % bar. A fault does fire on M5's own patches — Release macro 127 at 50 Hz reads **3.3270 / 3.3270 / 3.3481 / 3.3481 / 3.0276 / 3.0416 % → RED**, and unlike F5 that is a departure from the fixed time-constant patch the row is stated at, so it is a legitimate planted fault. §1's "as F5" should be replaced by it. |

**Six rows refuted: F3(c), F4, F5, O1, V3, V6.** Two are the same defect in
the *evidence* rather than in the class — F5 and V6 both call a reachable
Release position a planted fault — one is a confound the pack had already
found in the neighbouring clause (F3), one is a fault that cannot fire (O1),
one is a trait the class cannot reach (F4), and one is a bar that does not
survive a change of probe tone (V3).

**Refutation pass: run.** The §8 gate item and the `Refutation pass` line in
§1 can be ticked, at ten of sixteen demonstrated clauses surviving.
